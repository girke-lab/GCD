 ...
 */
public class StaticUnknownsDataView implements DataView
{
    List seq_ids;
    int hid;
    KeyType keyType;
         ...
         return new DefaultQueryWideView(){
            ...
            public void printGeneral(PrintWriter out, Search search, String pos,Map storage)
            {
                String link="DispatchServlet?hid="+hid+"&script=unknownsText&range=0-"+search.getResults().size()+
                                        "&dataType=UnknownGenes";
                out.println("Download data: &nbsp");
                out.println("<a href='"+link+"' >Unknown Genes</a>");
            }
...
//            out2.close();
    ...
    private void getAndPrintData(PrintWriter out, List ids)
    {
        Collection data ;//,go, blast, protomics, cluster, external, expSet;
        RecordFactory f=RecordFactory.getInstance();
        ...
        try{
            ...
            for(int j=0; j < ids.size(); j+=batchSize)
            {
                ...
                qp.setIds(ids.subList(j,end));
                data=f.getRecords(UnknownRecord.getRecordInfo(), qp);
                f.addSubType(data,ExternalUnknownRecord.getRecordInfo(),qp);        
